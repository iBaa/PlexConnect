...
def XML_PMS2aTV(PMS_address, path, options):
    ...
    if cmd.startswith('SettingsToggle:'):
        ...
        parts = opt.split('+')
        g_ATVSettings.toggleSetting(
            options['PlexConnectUDID'], parts[0].lower())
        parts1 = parts[1].split('_', 1)
        dir = parts1[0]
        cmd = parts1[1]
        XMLtemplate = dir + '/' + cmd + '.xml'
        dprint(__name__, 2,
               "ATVSettings->Toggle: {0} in template: {1}", parts[0], parts[1])
        path = ''  # clear path - we don't need PMS-XML
        ...
        info = urllib.request.urlopen(
            "https://youtube.com/get_video_info?html5=1&video_id=" + trailerID).read()
        ...
        (username, auth_token) = PlexAPI.MyPlexSignIn(
        ...
        if g_ATVSettings.getSetting(UDID, 'plexhome_enable') == 'True':
            tokenDict['PlexHome'] = g_ATVSettings.getSetting(
                UDID, 'plexhome_auth')
        PlexAPI.discoverPMS(
            UDID, g_param['CSettings'], g_param['IP_self'], tokenDict)
        ...
        if not PILBackgrounds.isPILinstalled() or \
           not options['aTVFirmwareVersion'] >= '6.0':
            dprint(
                __name__, 2, "disable fanart (PIL not installed or aTVFirmwareVersion<6.0)")
    ...
    while True:
        ...
        CommandCollection = CCommandCollection(
            options, PMSroot, PMS_address, path)
...
def XML_ExpandTree(CommandCollection, elem, src, srcXML):
    ...
    while True:
        ...
        for child in elem:
            res = XML_ExpandNode(CommandCollection, elem,
                                 child, src, srcXML, 'TEXT')
...
def XML_ExpandNode(CommandCollection, elem, child, src, srcXML, text_tail):
    if text_tail == 'TEXT':  # read line from text or tail
        ...
        dprint(__name__, 0,
               "XML_ExpandNode - text_tail badly specified: {0}", text_tail)
        return False
    ...
    while line != None:
        ...
        if cmd[-1] != ')':
            dprint(__name__, 0,
                   "XML_ExpandNode - closing bracket missing: {0} ", line)
        ...
        if hasattr(CCommandCollection, 'TREE_'+cmd):  # expand tree, work COPY, CUT
            ...
            try:
                ...
                res = getattr(CommandCollection, 'TREE_' +
                              cmd)(elem, child, src, srcXML, param)
            except:
                dprint(
                    __name__, 0, "XML_ExpandNode - Error in cmd {0}, line {1}\n{2}", cmd, line, traceback.format_exc())
        ...
        # check other known cmds: VAL, EVAL...
            ...
            dprint(__name__, 2,
                   "XML_ExpandNode - Stumbled over {0} in line {1}", cmd, line)
            pos = cmd_end
        else:
            dprint(
                __name__, 0, "XML_ExpandNode - Found unknown cmd {0} in line {1}", cmd, line)
...
def XML_ExpandAllAttrib(CommandCollection, elem, src, srcXML):
    ...
    line = elem.text
    if line != None:
        elem.text = XML_ExpandLine(
            CommandCollection, src, srcXML, line.strip())

    # unpack template commands in elem.tail
    line = elem.tail
    if line != None:
        elem.tail = XML_ExpandLine(
            CommandCollection, src, srcXML, line.strip())

    # unpack template commands in elem.attrib.value
    for attrib in elem.attrib:
        line = elem.get(attrib)
        elem.set(attrib, XML_ExpandLine(
            CommandCollection, src, srcXML, line.strip()))
...
def XML_ExpandLine(CommandCollection, src, srcXML, line):
    ...
    while True:
        ...
        if cmd[-1] != ')':
            dprint(__name__, 0,
                   "XML_ExpandLine - closing bracket missing: {0} ", line)
        ...
        if hasattr(CCommandCollection, 'ATTRIB_'+cmd):
            ...
            try:
                # expand any attributes in the parameter
                param = XML_ExpandLine(CommandCollection, src, srcXML, param)
                res = getattr(CommandCollection, 'ATTRIB_' +
                              cmd)(src, srcXML, param)
                line = line[:cmd_start] + res + line[cmd_end+2:]
                pos = cmd_start+len(res)
            except:
                dprint(
                    __name__, 0, "XML_ExpandLine - Error in {0}\n{1}", line, traceback.format_exc())
                line = line[:cmd_start] + \
                    "((ERROR:"+cmd+"))" + line[cmd_end+2:]

        elif hasattr(CCommandCollection, 'TREE_'+cmd):  # check other known cmds: COPY, CUT
            dprint(__name__, 2,
                   "XML_ExpandLine - stumbled over {0} in line {1}", cmd, line)
            pos = cmd_end
        else:
            dprint(
                __name__, 0, "XML_ExpandLine - Found unknown cmd {0} in line {1}", cmd, line)
            line = line[:cmd_start] + "((UNKNOWN:"+cmd+"))" + line[cmd_end+2:]
...
class CCommandHelper():
    ...
    def getParam(self, src, param):
        ...
        param = param.replace("7pmtimestamp", str(
            int(time.mktime(sevenDate.timetuple()))))
        param = param.replace("11pmtimestamp", str(
            int(time.mktime(elevenDate.timetuple()))))
    ...
    def getKey(self, src, srcXML, param):
        ...
        while '/' in attrib and el != None:
            ...
            if parts[0].startswith('#') and attrib[1:] in self.variables:
                ...
                el = el.find(g_ATVSettings.getSetting(
                    self.ATV_udid, parts[0][1:]))
            elif parts[0].startswith('%'):  # PMS property
                el = el.find(PlexAPI.getPMSProperty(
                    self.ATV_udid, self.PMS_uuid, parts[0][1:]))
            else:
                el = el.find(parts[0])
            attrib = parts[1]
        ...
        if attrib.startswith('#') and attrib[1:] in self.variables:
            ...
            res = PlexAPI.getPMSProperty(
                self.ATV_udid, self.PMS_uuid, attrib[1:])
            dfltd = False
    ...
    def getConversion(self, src, param):
        ...
        if conv != '':
            ...
            for part in parts:
                convstr = part.split('=')
                convlist.append(
                    (unquote_plus(convstr[0]), unquote_plus(convstr[1])))
    ...
    def applyMath(self, val, math, frmt):
        ...
        try:
            ...
            dprint(
                __name__, 0, "CCmds_applyMath: Error in math {0}, frmt {1}\n{2}", math, frmt, traceback.format_exc())
...
class CCommandCollection(CCommandHelper):
    ...
    def TREE_COPY_PLAYLIST(self, elem, child, src, srcXML, param):
        ...
        playType, leftover, dfltd = self.getKey(
            src, srcXML, leftover)  # Single (default), Continuous
        ...
        elemsSRC = []
        for elemSRC in src.findall(tag):
            child_key, leftover, dfltd = self.getKey(
                elemSRC, srcXML, param_key)
            if not key:
    ...
    def TREE_ADDXML(self, elem, child, src, srcXML, param):
        ...
        if PMS_address in ['all', 'owned', 'shared', 'local', 'remote']:
            # owned, shared PMSs
            type = PMS_address
            PMS = PlexAPI.getXMLFromMultiplePMS(
                self.ATV_udid, path, type, self.options)
        else:
            # IP:port or plex.tv
            auth_token = PlexAPI.getPMSProperty(
    ...
    def TREE_MEDIABADGES(self, elem, child, src, srcXML, param):
        ...
        container, leftover, dfltd = self.getKey(
            src, srcXML, param + "/container")
        vCodec, leftover, dfltd = self.getKey(
            src, srcXML, param + "/videoCodec")
        aCodec, leftover, dfltd = self.getKey(
            src, srcXML, param + "/audioCodec")
        channels, leftover, dfltd = self.getKey(
            src, srcXML, param + "/audioChannels")
        ...
        if resolution not in ['720', '1080', '2k', '4k']:
            attribs['src'] = g_param['baseURL'] + \
                '/thumbnails/MediaBadges/sd.png'
        else:
            attribs['src'] = g_param['baseURL'] + \
                '/thumbnails/MediaBadges/' + resolution + '.png'
        ...
        if vCodec == 'drmi' or aCodec == 'drms':
            ...
            attribs['src'] = g_param['baseURL'] + \
                '/thumbnails/MediaBadges/iTunesDRM.png'
            urlBadge = etree.SubElement(additionalBadges, "urlBadge", attribs)
        ...
        if container != '' and self.options['aTVFirmwareVersion'] >= '7.0':
            attribs['insertIndex'] = str(index)
            attribs['src'] = g_param['baseURL'] + \
                '/thumbnails/MediaBadges/' + container + '.png'
            urlBadge = etree.SubElement(additionalBadges, "urlBadge", attribs)
            index += 1
        # Video Codec
        if vCodec != '' and self.options['aTVFirmwareVersion'] >= '7.0':
            ...
            attribs['insertIndex'] = str(index)
            attribs['src'] = g_param['baseURL'] + \
                '/thumbnails/MediaBadges/' + vCodec + '.png'
            urlBadge = etree.SubElement(additionalBadges, "urlBadge", attribs)
            index += 1
        # Audio Codec
        if aCodec != '':
            attribs['insertIndex'] = str(index)
            attribs['src'] = g_param['baseURL'] + \
                '/thumbnails/MediaBadges/' + aCodec + '.png'
            urlBadge = etree.SubElement(additionalBadges, "urlBadge", attribs)
            index += 1
        # Audio Channels
        if channels != '':
            attribs['insertIndex'] = str(index)
            attribs['src'] = g_param['baseURL'] + \
                '/thumbnails/MediaBadges/' + channels + '.png'
            urlBadge = etree.SubElement(additionalBadges, "urlBadge", attribs)
        # Append XML
        child.append(additionalBadges)
    ...
    def ATTRIB_IMAGEURL(self, src, srcXML, param):
        ...
        if cmd_start > -1 and cmd_end > -1 and cmd_end > cmd_start:
            ...
            PMS_baseURL = PlexAPI.getPMSProperty(
                self.ATV_udid, PMS_uuid, 'baseURL')
            key = key[cmd_end+1:]

        AuthToken = PlexAPI.getPMSProperty(
            self.ATV_udid, PMS_uuid, 'accesstoken')
        ...
        parts = key.rsplit('.', 1)
        photoATVNative = parts[-1].lower() in ['jpg',
                                               'jpeg', 'tif', 'tiff', 'gif', 'png']
    ...
    def ATTRIB_VIDEOURL(self, src, srcXML, param):
        ...
        if not Video:
            dprint(__name__, 0,
                   "VIDEOURL - VIDEO element not found: {0}", param)
        ...
        if Media != None:
            ...
            videoATVNative = \
                ...
                Media.get('audioCodec', '-') in ("aac",
                                                 "drms")   # remove AC3 when Dolby Digital is Off

            # determine if Dolby Digital is active
            DolbyDigital = g_ATVSettings.getSetting(
                self.ATV_udid, 'dolbydigital')
            if DolbyDigital == 'On':
            ...
            if PlexAPI.getPMSProperty(self.ATV_udid, self.PMS_uuid, 'local') == '1':
                qLimits = qLookup[g_ATVSettings.getSetting(
                    self.ATV_udid, 'transcodequality')]
            else:
                qLimits = qLookup[g_ATVSettings.getSetting(
                    self.ATV_udid, 'remotebitrate')]
            ...
            subtitleIOSNative = \
                subtitleKey == '' and (subtitleCodec == "mov_text" or subtitleCodec ==
                                       "ttxt" or subtitleCodec == "tx3g" or subtitleCodec == "text")  # embedded
            subtitlePlexConnect = \
            ...
            dprint(__name__, 2, "subtitle: IOSNative - {0}, PlexConnect - {1}, DirectPlay - {2}",
                   subtitleIOSNative, subtitlePlexConnect, subtitleDirectPlay)
            ...
            if transcoderAction == 'DirectPlay' \
               ...
               subtitleDirectPlay:
                ...
                if Media.get('indirect', False):
                    ...
                    res, leftover, dfltd = self.getKey(
                        PMS.getroot(), srcXML, 'Video/Media/Part['+str(partIndex+1)+']/key')
                ...
                audio = {'boost': g_ATVSettings.getSetting(
                    self.ATV_udid, 'audioboost')}
                res = PlexAPI.getTranscodeVideoPath(
                    res, AuthToken, self.options, transcoderAction, qLimits, subtitle, audio, partIndex)

        else:
            dprint(__name__, 0,
                   "VIDEOURL - MEDIA element not found: {0}", param)
    ...
    def ATTRIB_episodestring(self, src, srcXML, param):
        ...
        out = self._("{0:0d}x{1:02d} {2}").format(
            int(parentIndex), int(index), title)
        return out
    ...
    def ATTRIB_BACKGROUNDURL(self, src, srcXML, param):
        ...
        res = g_param['baseURL']  # base address to PlexConnect
        res = res + PILBackgrounds.generate(self.PMS_uuid, key, auth_token, self.options['aTVScreenResolution'], g_ATVSettings.getSetting(
            self.ATV_udid, 'fanart_blur'), g_param['CSettings'])
