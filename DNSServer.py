...
def HostToDNS(Host):
    ...
    while i < len(DNSdata)-1:
        ...
        if next == -1:
            next = len(DNSdata)-1
        # python 2.6: DNSdata[i] = next-i-1
        DNSdata = DNSdata[:i] + chr(next-i-1) + DNSdata[i+1:]
        i = next
    ...
    while DNSdata[i] != '\0':
        ...
        if nlen & 0xC0:
            if followlink:
                Host = Host + \
                    DNSToHost(
                        DNSdata, ((ord(DNSdata[i]) & 0x3F) << 8) + ord(DNSdata[i+1]), True)+'.'
            break
        else:
...
def parseDNSdata(paket):
    ...
    for i in range(DNSstruct['head']['ancnt'] + DNSstruct['head']['nscnt'] + DNSstruct['head']['arcnt']):
        ...
        DNSstruct['resrc'][i]['ttl'] = (
            getWord(paket, adr+4) << 16)+getWord(paket, adr+6)
...
def encodeDNSstruct(DNSstruct):
    ...
    # resource records
    for i in range(DNSstruct['head']['ancnt'] + DNSstruct['head']['nscnt'] + DNSstruct['head']['arcnt']):
        # no 'packing'/link - todo?
        host = HostToDNS(DNSstruct['resrc'][i]['host'])
        DNS.extend(bytearray(host))
...
def Run(cmdPipe, param):
    ...
    try:
        ...
        dprint(
            __name__, 0, "Failed to create socket on UDP port {0}: {1}", cfg_Port_DNSServer, e)
        sys.exit(1)
    ...
    dprint(__name__, 0, "***")
    dprint(__name__, 0,
           "DNSServer: Serving DNS on {0} port {1}.", cfg_IP_self, cfg_Port_DNSServer)
    ...
    try:
        while True:
            ...
            try:
                ...
                # Opcode bits (query=0, inversequery=1, status=2)
                opcode = (ord(data[2]) >> 3) & 0x0F
                ...
                if domain in intercept:
                    ...
                    # 4:5 - QDCOUNT - should be 1 for this code
                    paket += data[4:6]
                    ...
                    # original query
                    paket += data[12:]
                    # pointer to domain name/original query
                    paket += '\xc0\x0c'
                    # response type, ttl and resource data length -> 4 bytes
                    paket += '\x00\x01\x00\x01\x00\x00\x00\x3c\x00\x04'
                    # 4bytes of IP
                    paket += str.join('', [chr(int(x))
                                      for x in cfg_IP_self.split('.')])
                    ...
                    try:
                        DNS_forward = socket.socket(
                            socket.AF_INET, socket.SOCK_DGRAM)
                        DNS_forward.settimeout(5.0)
                    except Exception as e:
                        dprint(
                            __name__, 0, "Failed to create socket for DNS_forward): {0}", e)
                        continue
                ...
                dprint(__name__, 1,
                       "Warning: DNS error ({0}): {1}", e.errno, e.strerror)
...
if __name__ == '__main__':
    ...
    param['baseURL'] = 'http://' + param['IP_self'] + \
        ':' + cfg.getSetting('port_webserver')
